...
def _redact_env_value(key: str, value: str) -> str:
    ...
    if any(hint in normalized for hint in _SECRET_ENV_HINTS):
        return "<redacted>"
    return value


def _sanitized_compose_env(env_values: dict[str, str]) -> dict[str, str]:
    """Build an explicit compose environment so shell secrets do not leak into config."""
    allowed_passthrough = {
        "HOME",
        "PATH",
        "TMPDIR",
    ...
    env = {key: value for key, value in os.environ.items() if key in allowed_passthrough}
    env.update(env_values)
    return env


def _write_env_file(path: Path, values: dict[str, str], *, force: bool) -> None:
    if path.exists() and not force:
        console.print(f"[red]Env file already exists:[/red] {path}")
    ...
    sections = [
        (
            ...
            [
                ...
                ("NANOBOT_STATE_DIR", values["NANOBOT_STATE_DIR"]),
                ("HEALTH_HTTP_PORT", values["HEALTH_HTTP_PORT"]),
                ("DOMAIN", values["DOMAIN"]),
                ("HEALTH_ONBOARDING_BASE_URL", values["HEALTH_ONBOARDING_BASE_URL"]),
            ],
        ),
        (
            "Local model provider",
            [
                ("NANOBOT_AGENTS__DEFAULTS__PROVIDER", values["NANOBOT_AGENTS__DEFAULTS__PROVIDER"]),
                ("NANOBOT_AGENTS__DEFAULTS__MODEL", values["NANOBOT_AGENTS__DEFAULTS__MODEL"]),
                ("OLLAMA_API_BASE", values["OLLAMA_API_BASE"]),
                ("NANOBOT_SKIP_SETUP_PROVIDER", values["NANOBOT_SKIP_SETUP_PROVIDER"]),
            ],
        ),
        (
            "Secrets generated for this local file",
            [
                ("HEALTH_VAULT_KEY", values["HEALTH_VAULT_KEY"]),
        ...
        (
            "Image and release metadata",
            [
                ("NANOBOT_HEALTH_INSTANCE_IMAGE", values["NANOBOT_HEALTH_INSTANCE_IMAGE"]),
    ...
    lines = [
        "# Generated by `healthclaw init-local`.",
        "# Safe for local development. Do not paste real provider keys into chat or logs.",
        "",
    ]
...
def init_local(
    env_file: str = typer.Option(".env.local", "--env-file", help="Env file to create"),
    force: bool = typer.Option(False, "--force", help="Replace an existing env file"),
    model: str = typer.Option("gemma:7b", "--model", help="Default Ollama model"),
    http_port: int = typer.Option(18080, "--http-port", help="Local Healthclaw HTTP port"),
    state_dir: str = typer.Option(".local/nanobot-state", "--state-dir", help="Local runtime state directory"),
) -> None:
    """Create a safe local self-host env file without copying shell secrets."""
    try:
        from cryptography.fernet import Fernet
    except ImportError as exc:
        console.print("[red]cryptography is required to generate HEALTH_VAULT_KEY.[/red]")
        raise typer.Exit(1) from exc

    path = Path(env_file).expanduser().resolve()
    values = {
        "COMPOSE_PROJECT_NAME": "healthclaw-local",
        "NANOBOT_STATE_DIR": state_dir,
        "HEALTH_HTTP_PORT": str(http_port),
        "DOMAIN": "localhost",
        "HEALTH_ONBOARDING_BASE_URL": f"http://localhost:{http_port}",
        "NANOBOT_AGENTS__DEFAULTS__PROVIDER": "ollama",
        "NANOBOT_AGENTS__DEFAULTS__MODEL": model,
        "OLLAMA_API_BASE": "http://host.docker.internal:11434",
        "NANOBOT_SKIP_SETUP_PROVIDER": "false",
        "HEALTH_VAULT_KEY": Fernet.generate_key().decode("ascii"),
        "POSTGRES_PASSWORD": secrets.token_urlsafe(24),
        "NANOBOT_HEALTH_INSTANCE_IMAGE": "healthclaw-local-orchestrator:latest",
    }
    _write_env_file(path, values, force=force)
    console.print(f"[green]✓[/green] Created {path}")
    console.print("[dim]No provider API keys were copied from your shell environment.[/dim]")
    console.print(f"Run: [cyan]docker compose --env-file {path} up -d --build postgres redis orchestrator worker[/cyan]")
...
def _http_get_json(url: str, *, timeout: float = 2.0) -> dict[str, Any] | None:
    try:
        with urlopen(url, timeout=timeout) as response:  # noqa: S310 - user-configured local diagnostics
...
@app.command("doctor")
def doctor(
    env_file: str = typer.Option(".env.local", "--env-file", help="Env file to validate"),
    compose_file: str = typer.Option("docker-compose.yml", "--compose-file", help="Compose file to validate"),
) -> None:
    """Validate a local Healthclaw self-host setup with redacted diagnostics."""
    ...
    console.print(f"{__logo__} Healthclaw doctor\n")

    if env_path.exists():
        ok(f"Env file found: {env_path}")
    else:
        fail(f"Env file missing: {env_path}")

    required = ("HEALTH_VAULT_KEY", "POSTGRES_PASSWORD", "HEALTH_ONBOARDING_BASE_URL")
    for key in required:
    ...
    provider = env_values.get("NANOBOT_AGENTS__DEFAULTS__PROVIDER", "ollama").strip().lower()
    model = env_values.get("NANOBOT_AGENTS__DEFAULTS__MODEL", "gemma:7b").strip()
    console.print("\n[bold]Redacted env summary[/bold]")
    for key in sorted(env_values):
        if key in required or key.endswith("_API_KEY") or key.endswith("_TOKEN"):
            console.print(f"  {key}={_redact_env_value(key, env_values[key])}")
    ...
    if docker_path and compose_path.exists():
        compose_env = _sanitized_compose_env(env_values)
        result = subprocess.run(
            [docker_path, "compose", "--env-file", str(env_path), "-f", str(compose_path), "config"],
        ...
        fail(f"Compose file missing: {compose_path}")

    if provider == "ollama":
        ollama_base = env_values.get("OLLAMA_API_BASE", "http://host.docker.internal:11434")
        local_base = ollama_base.replace("host.docker.internal", "localhost").rstrip("/")
        tags = _http_get_json(f"{local_base}/api/tags")
        if tags is None:
            warn(f"Ollama was not reachable at {local_base}; start Ollama before first local run")
        else:
            ...
            if model in names or any(name.startswith(f"{model}:") for name in names):
                ok(f"Ollama model appears installed: {model}")
            else:
                warn(f"Ollama is reachable, but {model} was not listed; run `ollama pull {model}`")

    try:
        port = int(env_values.get("HEALTH_HTTP_PORT", "18080") or "18080")
    except ValueError:
        port = 18080
        warn("HEALTH_HTTP_PORT is not an integer; defaulting diagnostics to 18080")
    if _check_port_open("127.0.0.1", port):
        healthz = _http_get_json(f"http://127.0.0.1:{port}/healthz")
        if healthz and healthz.get("ok"):
            ok(f"Healthclaw /healthz is responding on port {port}")
        else:
            warn(f"Port {port} is open but /healthz did not return ok")
    else:
        warn(f"Healthclaw is not currently listening on port {port}; start the stack when ready")

    if failures:
        console.print(f"\n[red]{len(failures)} failure(s), {len(warnings)} warning(s).[/red]")
        raise typer.Exit(1)
    console.print(f"\n[green]Doctor passed with {len(warnings)} warning(s).[/green]")
